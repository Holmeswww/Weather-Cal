...
// --- Construct the widget layout from Gemini's response ---

// 1. Split the layout items from Gemini into an array
...
// 3. Build the strings for the left and right content columns
...
// 4. Prepare the message from Gemini
const safeMessage = geminiData.message.replace(/"/g, '\\"');

// 5. Assemble the final layout
// This creates a top-level column. The first element is the message text.
// The second element is a row containing the two columns for the other widget items.
const layout = `
column
  text(${safeMessage})
  space(10)
  row${leftColumnString}${rightColumnString}
`;
